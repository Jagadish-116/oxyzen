 ...
 */
export async function searchSongs(query, page = 1, limit = 20, resolveStreams = false) {
    if (!query || !query.trim()) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
        try {
            ...
            const formatPromises = rawResults.map((raw) => formatSongObject(raw, resolveStreams));
...
export async function getVibeRecommendations(songId, artist, title, language) {
    try {
        ...
        if (songId)
            seenIds.add(songId);
        if (title)
            seenTitleKeys.add(getNormalizedTitleKey(title));
...
export async function getExploreFeed(profile, currentTrack) {
    try {
        ...
        const sections = [];
        const dedupeTracks = (list, limit = 30, targetLang) => {
            const out = [];
...
export async function importPlaylistFromUrl(rawUrl) {
    ...
    if (url.includes('spotify.com')) {
        ...
        const rawTrackList = entity.trackList || entity.tracks || [];
        ...
        for (let i = 0; i < rawTrackList.length; i += 3) {
            ...
            const batchResults = await Promise.all(batch.map(async (st) => {
                ...
                const subtitle = st.subtitle || (st.artists ? st.artists.map((a) => a.name).join(' ') : '');
