 ...
 */
export async function searchSongs(query: string, page = 1, limit = 20): Promise<{ results: FormattedSong[]; total: number; start: number }> {
  if (!query || !query.trim()) {
  ...
  try {
    ...
    const formatPromises = rawResults.map((raw: any) => formatSongObject(raw, true));
 ...
 */
export async function getVibeRecommendations(songId?: string, artist?: string, title?: string, language?: string): Promise<FormattedSong[]> {
  try {
...
export async function getExploreFeed(
  profile?: { languages?: string[]; history?: any[] },
  ...
  try {
    ...
    const popularTracks: FormattedSong[] = [];
    const popularSeen = new Set<string>();
    ...
    let historyRecommendations: FormattedSong[] = [];
    let topArtistsInHistory: string[] = [];
 ...
 */
export async function getMoodFeed(moodKey: string, languages: string[] = ['Telugu', 'Hindi', 'English']): Promise<{ mood: MoodCategory; tracks: FormattedSong[] }> {
